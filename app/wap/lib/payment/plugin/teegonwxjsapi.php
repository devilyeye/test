 ...
 */
final class wap_payment_plugin_teegonwxjsapi extends ectools_payment_app  implements ectools_interface_payment_app {

	/**
	 * @var string 支付方式名称
	 */
    public $name = '天工支付（微信jsapi）';//快钱网上支付
    /**
     * @var string 支付方式接口名称
     */
     ...
     * @var string 支付方式key
     */
	public $app_key = 'teegonwxjsapi';
	/**
	 * @var string 中心化统一的key
	 */
	public $app_rpc_key = 'teegonwxjsapi';
	/**
	 * @var string 统一显示的名称
	 */
    public $display_name = '天工支付(wap微信';
    /**
	 * @var string 货币名称
	 */
    public $curname = 'CNY';
	...
	public function __construct($app)
	{
		...
		$this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_teegonwxjsapi', 'callback');
        ...
        $this->submit_url = 'http://api.teegon.com/v1/charge';
...
//        $data = curl_exec($ch);
//        curl_close($ch);
//        error_log(print_r($this->submit_url,1),3,DATA_DIR.'/log.log');
//        error_log(print_r($data,1),3,DATA_DIR.'/log.log');
//        error_log(print_r($return,1),3,DATA_DIR.'/log.log');
//        exit;
	  $result = $this->call('post',$this->submit_url,$return);
            echo $this->get_html($result['result']['action']['params']);exit;
        }else{
            return false;
        }
    }
	...
	$result = curl_exec($ch);
        ...
        //}
        curl_close($ch);
        //error_log(print_r(json_decode($result),1),3,DATA_DIR.'/log.log');
        return json_decode($result,1);
    }


    protected function get_html($str=''){
...
";
    ...
    function callback(&$in){
		...
		$ret['pay_app_id'] = "teegonwxjsapi";
		$ret['pay_type'] = 'online';
		$ret['memo'] = 'teegonwxjsapi';
